...
function useLiveScores() {
  const [players, setPlayers] = useState(competitionYears[t.year]?.leaderboard ?? []);
...
export default function App() {
  return <><Header/><main id="top">
    <section className="hero">
      ...
      <div className="hero-facts"><div><CalendarDays/><span><small>When</small>{t.event.dates}</span></div><div><MapPin/><span><small>Where</small>{t.event.location}</span></div><div><Trophy/><span><small>Status</small>{t.status} · <a href={t.registrationUrl} target="_blank" rel="noreferrer">Live Leaderboard</a></span></div><div><DollarSign/><span><small>Entry fee</small>{t.event.price}</span></div></div>
    </section>
